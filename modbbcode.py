...
smile_start = '<img src="http://static.rutracker.cc/smiles/'
smile_end = '.gif" alt="" />'
...
zam = {'[b]':'<span class="post-b">', '[/b]':'</span>',
    ...
    '[list]':'<ol class="post-ul">','[list=1]':'<ul class="post-ul">', '[/list]':'</ol>',
...
tags=[('[b]','[/b]'),('[i]','[/i]'),('[u]','[/u]'),('[s]','[/s]'),('[list]','[/list]'),
      ...
      ('[pre','[/pre]'),('[code','[/code]'),('[url','[/url]'),                      # ('[spoiler','[/spoiler]'),
...
def zam_sp(intext,n):
    header='''<div class="sp-wrap"><div class="sp-head folded"><span>%s</span></div><div class="sp-body" style="display: none;">'''
    
    spoilers=re.findall(r'(\[spoiler=\")(.*?)(\"\])',intext,re.M+re.I+re.S)
    for item in spoilers:
        par=item[1]
        intext=intext.replace('[spoiler="{0}"]'.format(par),(header % (par)))
    intext=intext.replace('[/spoiler]','</div></div>')
    return intext
...
def bbcode2html(intext):
    Dictag={'IMG':('<img class="postImg" align="%s" alt=" " src="%s','">'),                              #
    ...
    intext=intext.replace('[hr]','<hr class="post-hr">')
...
if __name__ == '__main__':
    ...
    """
    s= # внести id #
    spoilers=re.findall(r'(\[spoiler=\")(.*?)(\"\])',s,re.S)
    aaa=bbcode2html(s)
    print(aaa)
    """
